...
class AddingHandler:
    ...
    async def get_data(update, context) -> None:
        ...
        if len(splited_text) > 1:
            ...
            if len(checking_uniqueness) == 0:
                await update.message.reply_text(adding_confirmation, reply_markup=yn_keyboard()) 
    ...
    async def get_adding_confirmation(update, context):
        ...
        if user_text == "yes":
            ...
            await update.message.reply_text("The word was added successfully.\nCongratulations!!!", reply_markup=main_keyboard())
        else:
            await update.message.reply_text("You cancelled adding", reply_markup=main_keyboard())
        return ConversationHandler.END
...
class PracticingHandler:
    ...
    async def practice_checking(update, context):
        try:
            ...
            if user_text == Practice.first_round()[0][3].lower():
                Practice.first_round_updater("right")
                ...
                Practice.first_round_updater("wrong")
...
class DeletionHandler():
    ...
    async def get_data_for_deletion(update, context):
        ...
        try:
            ...
            await update.message.reply_text(Deletion(words_ids=splited_user_text).obtainig_for_deletion(), reply_markup=yn_keyboard())
    ...
    async def get_deletion_confirmation(update, context):
        ...
        if user_text == "yes":
            ...
            await update.message.reply_text("The word is deleted", reply_markup=main_keyboard())
        else:
            Deletion(command="no").completing_deletion()
            await update.message.reply_text("You cancelled the deletion", reply_markup=main_keyboard())
        return ConversationHandler.END
