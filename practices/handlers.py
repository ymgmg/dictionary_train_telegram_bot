...
class PracticingHandler:
    async def practice_start(update, context):
        ...
        try:
            if PracticeOneToFour(chat_id=chat_id).checking_availability() >= 4:
                ...
                await update.message.reply_text("Please choose amount of words you want to train",
    ...
    async def practice_select_number(update, context):
        user_text = update.message.text
        chat_id = update.message.chat.id

        try:
            PracticeOneToFour(chat_id=chat_id, session_amount=int(user_text)).creating_practice_table()
    ...
    async def practice_checking(update, context):
        ...
        chat_id = update.message.chat.id
        try:
            checking_answer = PracticeOneToFour(chat_id=chat_id, answer_to_check=user_text).checking_for_correctness()
            ...
            practice_item = PracticeOneToFour(chat_id=chat_id).process()
            print(practice_item)
            question_item = practice_item["word"] if practice_item["round_number"]==1 else practice_item["translate"]
            options_list = practice_item["choosed_options"]
            await update.message.reply_text(f"Write translate to the word:\n{question_item}",
                reply_markup=ReplyKeyboard.options_keyboard(options_list))
            return "practice"
            ...
            try:
                result = round(stat["right_answers"] / stat["all_answers"] * 100, 2)
            except ZeroDivisionError:
                result = 0
            await update.message.reply_text("Practice is over")
            await update.message.reply_text(f"Your result is {result}%", reply_markup=ReplyKeyboard.main_keyboard())
            return ConversationHandler.END
