...
class BotCommands:
    ...
    async def main_page(update, context):
        await update.message.reply_text("You are at the main page",
        	reply_markup=ReplyKeyboard.main_keyboard())

    async def dict_shower(update, context) -> None:
        chat_id = update.message.chat.id
        await update.message.reply_text(db_content_shower(chat_id=chat_id, command="without_translate"))

    async def echo(update, context) -> None:
        await update.message.reply_text("it doesn't look like any of our commands",
        	reply_markup=ReplyKeyboard.main_keyboard())

    async def necessity_counter(update, context):
        await update.message.reply_text(show_necessary_words(update.message.chat.id))
